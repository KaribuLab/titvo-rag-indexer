...
def main():
    ...
    LOGGER.debug("Starting rag-indexer with repo=%s, commit=%s, branch=%s", repo_url, commit_sha, branch)

    if repo_url is None:
        raise ValueError("TITVO_REPO_URL is not set")
    if commit_sha is None and branch is None:
        raise ValueError("Either TITVO_COMMIT_SHA or TITVO_BRANCH must be set")

    # Si se proporciona branch, ignorar commit_sha (full index)
    if branch is not None:
        LOGGER.info("Branch provided (%s), ignoring commit_sha for full index mode", branch)
        commit_sha = None
    ...
    code_splitter = LangChainCodeSplitter(chunk_size=chunk_size, chunk_overlap=chunk_overlap)
