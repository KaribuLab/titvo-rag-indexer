...
class S3ArtifactStoreAdapter(IArtifactStorePort):
    ...
    def upload_db(self, repository_url: str, commit_sha: str, db_path: str) -> None:
        ...
        # Upload to commit-specific path
        commit_key = f"{repo_path}/{commit_sha}/index.db"
        ...
        meta_key = f"{repo_path}/{commit_sha}/meta.json"
        ...
        # Copy to latest/
        latest_db_key = f"{repo_path}/latest/index.db"
        latest_meta_key = f"{repo_path}/latest/meta.json"

        LOGGER.info("Copying to s3://%s/%s/latest/", bucket, repo_path)
        ...
        LOGGER.info("Successfully uploaded database for %s@%s", repository_url, commit_sha[:7])

    def download_latest_db(self, repository_url: str) -> Optional[str]:
        bucket = self._get_bucket_name()
        repo_path = self._build_repo_path(repository_url)
        key = f"{repo_path}/latest/index.db"
        ...
        try:
            ...
            if error_code == "404" or error_code == "NoSuchKey":
                LOGGER.debug("Latest DB not found for %s", repository_url)
                return None
    ...
    def get_latest_commit_sha(self, repository_url: str) -> Optional[str]:
        bucket = self._get_bucket_name()
        repo_path = self._build_repo_path(repository_url)
        key = f"{repo_path}/latest/meta.json"
        ...
        try:
            ...
            error_code = e.response.get("Error", {}).get("Code", "Unknown")
            if error_code == "404" or error_code == "NoSuchKey":
                LOGGER.debug("Latest meta.json not found for %s", repository_url)
                return None
