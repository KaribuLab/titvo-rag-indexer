terraform {
  source = "git::https://github.com/KaribuLab/terraform-aws-ecs.git?ref=v0.1.0"
}

locals {
  serverless    = read_terragrunt_config(find_in_parent_folders("serverless.hcl"))
  task_name     = "${local.serverless.locals.service_name}-ecs-${local.serverless.locals.stage}"
